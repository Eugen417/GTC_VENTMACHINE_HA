...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    sensors = [
        ...
        ("gtc_filter_state", 14, "as_is", PERCENTAGE, "Загрязнение фильтра"),
        ("gtc_current_fan_speed", 25, "as_is", None, "Текущая скорость"),
        ("gtc_target_fan_speed", 32, "as_is", None, "Целевая скорость"),
        ("gtc_operation_stage", 3, "state_map", None, "Этап работы")
    ]
...
class GTCSensor(SensorEntity):
    ...
    def __init__(self, hub, entry, name, address, mode, unit, friendly_name):
        self._hub = hub
        self._address = address
        self._mode = mode
        self._attr_native_unit_of_measurement = unit
        self._attr_name = friendly_name
        self._attr_unique_id = f"gtc_final_v15_{name}"
    ...
    def native_value(self):
        ...
        # Обработка температур со знаком (минус)
        if self._mode == "temp_01":
            if val > 32767:
                val -= 65536
            return round(float(val) * 0.1, 1)

        # Текстовые этапы работы
